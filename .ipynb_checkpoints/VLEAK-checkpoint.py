...
class snn_hmax(nn.Module):
    def __init__(self):
        ...
        self.fully_connected1 = nn.Linear(864, 128)
        self.relu = nn.ReLU()
...
if inp=="None":
    ...
    model = torch.load(f"model{inp}.pt").to(device)
...
weights_updates = []
timing = []
yy_record = torch.zeros(108,48, batch_size, model.num_capsule_channels, 31,31)
...
def forward(self, input_spikes, record=True):
    ...
    im_size=31
    container = torch.zeros(num_steps, bat_size, self.num_capsule_channels, 3,3).to(device)
    ...
    for cap_chan in range(self.num_capsule_channels):
        ...
        for jjj, batch_step in enumerate(input_spikes):
            ...
            if record:
                yy_record[self.counter, jjj, :len(c1_reduced), cap_chan] = chann
            
            xx = self.stdp_inshallah(torch.softmax(capsule.to(device), dim=-1))
            xx = self.c2(xx.reshape(len(xx),6,6))
            #cond = (xx.mean((1,2))>xx.mean((0,1,2)))
            #container[jjj, cond, cap_chan] = xx[cond]
            container[jjj, :, cap_chan] = xx
    if record:
        self.counter = self.counter+1
    container = container[(container.sum([1,2,3,4]).sort(descending=True)[1][0:24]).sort()[0]]
...
for epoch in range(num_epochs):
    ...
    for i, (images, labels) in enumerate(tqdm(train_loader, desc=f"Epoch {epoch+1}", position=0, leave=True)):  
        ...
        ims = pos_neg_to_frame(images)[50:146:2].to(device)
        ...
        if i == 0:
            with torch.no_grad():
                ...
                for j, (test_images, test_labels) in enumerate(tqdm(train_loader, desc="Testing")):
                    if j == num_evaluate:
                        break
                    ims = pos_neg_to_frame(test_images)[50:146:2].to(device)
